...
class TestAppEndPoints(TestCase):
    ...
    @mock.patch("open_note_scanner.pdf_server.api.controller.generate_pdf")
    def test_valid_urls(self, mock_generate_pdfs):
        ...
        qr_data = [
            "{}{:03}".format(size, index) for index, size in enumerate(paper_sizes)
        ]
        int_pages = ["10", "25"]
    ...
    def test_invalid_urls(self):
        ...
        paper_sizes = ["A4", "letter"]
        qr_data = [
            "{}{}".format(size[:1], index) for index, size in enumerate(paper_sizes)
        ]
        int_pages = ["10", "25"]
